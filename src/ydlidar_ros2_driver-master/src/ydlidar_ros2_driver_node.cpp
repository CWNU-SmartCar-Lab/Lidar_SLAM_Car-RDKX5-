...
int main(int argc, char *argv[]) {
  ...
  node->declare_parameter<std::string>("port", "/dev/ydlidar"); // 加入了默认值
  node->get_parameter("port", str_optvalue);
  ///lidar port
  laser.setlidaropt(LidarPropSerialPort, str_optvalue.c_str(), str_optvalue.size());

  ///ignore array
  node->declare_parameter<std::string>("ignore_array", ""); // 加入了默认值
  node->get_parameter("ignore_array", str_optvalue);
  laser.setlidaropt(LidarPropIgnoreArray, str_optvalue.c_str(), str_optvalue.size());

  std::string frame_id;
  node->declare_parameter<std::string>("frame_id", "laser_frame"); // 加入了默认值
  node->get_parameter("frame_id", frame_id);

  //////////////////////int property/////////////////
  int optval;
  /// lidar baudrate
  node->declare_parameter<int>("baudrate", 512000); // 加入了默认值
  ...
  node->declare_parameter<bool>("fixed_resolution", false); // 加入了默认值，解决崩溃！
  ...
  node->get_parameter("angle_max", d_optvalue);
  f_optvalue = static_cast<float>(d_optvalue);
  laser.setlidaropt(LidarPropMaxAngle, &f_optvalue, sizeof(float));

  node->declare_parameter<double>("angle_min", -180.0);
  node->get_parameter("angle_min", d_optvalue);
  f_optvalue = static_cast<float>(d_optvalue);
  laser.setlidaropt(LidarPropMinAngle, &f_optvalue, sizeof(float));

  /// unit: m
  node->declare_parameter<double>("range_max", 64.0);
  node->get_parameter("range_max", d_optvalue);
  f_optvalue = static_cast<float>(d_optvalue);
  laser.setlidaropt(LidarPropMaxRange, &f_optvalue, sizeof(float));

  node->declare_parameter<double>("range_min", 0.1);
  node->get_parameter("range_min", d_optvalue);
  f_optvalue = static_cast<float>(d_optvalue);
  laser.setlidaropt(LidarPropMinRange, &f_optvalue, sizeof(float));

  /// unit: Hz
  node->declare_parameter<double>("frequency", 10.0);
  node->get_parameter("frequency", d_optvalue);
  f_optvalue = static_cast<float>(d_optvalue);
